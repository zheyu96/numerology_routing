#include "MyAlgo1_time.h"

MyAlgo1_time::MyAlgo1_time(Graph graph, vector<pair<int, int>> requests, double _epsilon):
    AlgorithmBase(graph, requests) {
    algorithm_name = "MyAlgo1_time" + to_string(_epsilon);
    epsilon = _epsilon;
}

void MyAlgo1_time::variable_initialize() {
    // m = i + vt
    // x(i, m) = 0
...
Shape_vector MyAlgo1_time::separation_oracle() {
...
pair<Shape_vector, double> MyAlgo1_time::find_min_shape(int src, int dst) {
...
double MyAlgo1_time::recursion_calculate_min_shape(int left, int right, int t, vector<int> &path) {
    if(t <= 0) return INF;
...
Shape_vector MyAlgo1_time::recursion_find_shape(int left, int right, int t, vector<int> &path) {
...
void MyAlgo1_time::run() {
    ...
    res["actual_succ_request_cnt"] = graph.get_actual_succ_request_cnt();
