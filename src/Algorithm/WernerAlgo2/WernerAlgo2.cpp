    ...
    while (round-- && !requests.empty()) {
        ...
        for(pair<double, Shape_vector> P : shapes) {
            ...
            if(resource_ok) {
                ...
                // routing trace CSV（per-request 對照用，接受的都記）
                {
                    Shape_vector sv_acc = shape.get_node_mem_range();
                    vector<int> pr_acc = shape.get_link_purify_rounds();
                    ...
                    log_routing_trace(request_index, sv_acc.front().first, sv_acc.back().first,
                                      "accepted", sv_acc, pr_acc, fid_acc, prob_acc);
                }
